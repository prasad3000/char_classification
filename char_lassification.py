...
print(unicode_to_ascii('Ślusàrsk_'))
...
# Turn a word into a <word_length x 1 x n_letters>,
...
print(word_to_tensor('Jones').size())
...
input = Variable(word_to_tensor('Albert'))
...
def random_training_pair():                                                                                                               
    ...
    word = random.choice(category_lines[category])
    category_tensor = Variable(torch.LongTensor([all_categories.index(category)]))
    word_tensor = Variable(word_to_tensor(word))
...
def predict(input_word, n_predictions=3):
    print('\n> %s' % input_word)
    output = evaluate(Variable(word_to_tensor(input_word)))
